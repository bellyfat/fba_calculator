...
class TestingPackages(unittest.TestCase):
    ...
    def test_small_stnd_media(self):
        ...
        package = FBACalculator(l, w, h, wt, is_media=True)
        fees = package.fees()
        self.assertEqual(float(fees), 1.56)
    ...
    def test_special_oversize(self):
        ...
        package = FBACalculator(l, w, h, wt, is_media=False)
        fees = package.fees()
        self.assertEqual(float(fees), 135.11)
